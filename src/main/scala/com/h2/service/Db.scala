...
trait ProductDb {
  ...
  def numDepositProducts: Int = depositProducts.size
...
trait AccountDb {
  ...
  def saveDepositAccount(account: DepositsAccount): Unit = depositAccounts += (account.id -> account)
  ...
  def numDepositAccounts: Int = depositAccounts.size
